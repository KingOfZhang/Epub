...
- ( NSMutableDictionary *)sendPay_demo:(NSString*)orderid price:(NSString *)price name:(NSString *)name
{
    //订单标题，展示给用户
...
//        order_name = @"预约宠物店服务";
//    }else{
//        order_name    = [[NSUserDefaults standardUserDefaults] objectForKey:@"name"];
//    }
    
    
    //订单金额,单位（分）
    double num = [price doubleValue];
    int totalPrice = num*100;
    NSString *order_price = [NSString stringWithFormat:@"%d",totalPrice];
...
//    NSString *noncestr  = @"";
    ...
    [packageParams setObject: orderno           forKey:@"nonce_str"];   //随机串
    ...
    if ( prePayid != nil) {
        ...
        [signParams setObject: orderid    forKey:@"noncestr"];
