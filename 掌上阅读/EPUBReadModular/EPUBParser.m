    ...
    if (xmlData) {
        ...
        if ([err code] == 0)
        {
            ...
            for (GDataXMLElement *navPoint in navPoints)
            {
                ...
                NSArray *ppp= [navPoint nodesForXPath:@"ncx:navPoint" namespaces:dictNameSpace error:&errXPath];
                NSLog(@"ppp = %@",ppp);
                if (ppp.count > 0) {
                    for (GDataXMLElement *myPoint in ppp) {
                        NSString *myid=[[myPoint attributeForName:@"id"] stringValue];
                        
                        NSString *myplayOrder=[[myPoint attributeForName:@"playOrder"] stringValue];
                        NSArray *textArr=[myPoint nodesForXPath:@"ncx:navLabel/ncx:text" namespaces:dictNameSpace error:nil];
                        
                        NSString *myncx_text= @"";
                        if ([textArr count]>0) {
                            GDataXMLElement *label=textArr[0];
                            myncx_text=[label stringValue];
                        }
                        NSArray *contentArr=[myPoint nodesForXPath:@"ncx:content" namespaces:dictNameSpace error:nil];
                        NSString *myncx_src=@"";
                        if ([contentArr count]>0) {
                            GDataXMLElement *mynodeContent=contentArr[0];
                            NSLog(@"要取的地方 =%@",mynodeContent);
                            myncx_src=[[mynodeContent attributeForName:@"ncx:src"] stringValue];
                        ...
                        NSMutableDictionary *oneChapter=[NSMutableDictionary dictionary];
                        [oneChapter setObject:[myid length]>0?myid:@"" forKey:@"id"];
                        [oneChapter setObject:[myplayOrder length]>0?myplayOrder:@"" forKey:@"playOrder"];
                        [oneChapter setObject:[myncx_text length]>0?myncx_text:@"" forKey:@"text"];
                        [oneChapter setObject:[myncx_src length]>0?myncx_src:@"" forKey:@"src"];
...
-(NSMutableArray*)epubpageID:(NSString*)ncxFilePath
{
    //根据 ncx文件， 得到目录信息
    NSMutableArray *arrCatalog=nil;
    NSData *xmlData=[[NSData alloc] initWithContentsOfFile:ncxFilePath];
    ...
    if (xmlData) {
        ...
        if ([err code] == 0)
        {
            ...
            for (GDataXMLElement *navPoint in navPoints)
            {
                ...
                [page1 setObject:id1 forKey:@"id"];
                [page1 setObject:ncx_src forKey:@"href"];
                
                [arrCatalog addObject:page1];
                ...
                if (ppp.count > 0) {
                    for (GDataXMLElement *myPoint in ppp) {
                        ...
                        NSMutableDictionary *page1=[NSMutableDictionary dictionary];
                        [page1 setObject:myid forKey:@"id"];
                        [page1 setObject:myncx_src forKey:@"href"];
