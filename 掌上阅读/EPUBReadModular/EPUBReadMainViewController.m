...
-(void)initAfter
{
    ...
    if (self.fileInfo)
    {
        ...
        NSLog(@"目录信息%@",self.epubCatalogs);
        //页码索引
        self.epubPageRefs=[self.epubParser epubPageRefWithOpfFile:self.opfFilePath];
        NSLog(@"页码索引%@",self.epubPageRefs);
        
        //页码信息
        self.epubPageItems=[self.epubParser epubPageItemWithOpfFile:self.ncxFilePath];
        NSLog(@"页码信息%@",self.epubPageItems);
        
        
        NSLog(@"钱%@",_epubPageItems);
        NSMutableArray *zijiArr = [[NSMutableArray alloc]init];
        
        self.epubPageRefs = [self.epubParser epubpageID:self.ncxFilePath];
...
-(NSString*)pageURLWithPageRefIndex:(NSInteger)pageRefIndex
{
    ...
    if (pageRefIndex <0 || pageRefIndex >= [self.epubPageRefs count]) {
     ...
     NSLog(@"pageRefIndex%lo",pageRefIndex);
    NSString *pageRef=[self.epubPageRefs objectAtIndex:pageRefIndex];
...
{
    //跳转页码
    NSLog(@"跳转页pageRefIndex %lo",pageRefIndex);
    NSLog(@"跳转页offYIndexInPage %lo",offYIndexInPage);
...
-(void)showCatalog
{
    ...
    popVC.backBlock=^(NSMutableDictionary *para1){
        ...
        if (para1) {
            ...
            NSLog(@"标题啊  %@",[para1 objectForKey:@"text"]);
            [self searchBack:[para1 objectForKey:@"text"]];
...
-(void)searchBack:(NSString *)searchText{
    //在这里进行搜索位置，进行跳转
    NSLog(@"不执行啊，你大爷的");
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        XWEPUBSearchJieViewController *popVC=[[XWEPUBSearchJieViewController alloc] init];
        popVC.epubVC=self;
        popVC.searText = searchText;
        popVC.backBlock=^(NSMutableDictionary *para1){
        
        
            NSLog(@"完美的体现--%@",para1);
            NSInteger   OffYIndexInPage = [[para1 objectForKey:@"OffYIndexInPage"] integerValue];
            NSInteger   PageRefIndex = [[para1 objectForKey:@"PageRefIndex"] integerValue];
