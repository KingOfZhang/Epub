...
@property (nonatomic,assign) BOOL isDetailJie;
@end
...
//        NSArray *forArr = [pageURL componentsSeparatedByString:@"#"];
//        NSLog(@"forArr %@",forArr);
//        if (forArr.count > 1) {
//            _isDetailJie = YES;
//            
...
//        }
    ...
    NSInteger currentOffCountInPage=[[self.epubVC.dictPageWithOffYCount objectForKey:[NSString stringWithFormat:@"%@",@(self.pageRefIndex)]] integerValue];
//
    NSLog(@"currentOffCountInPage %lo",currentOffCountInPage);
    
    NSLog(@"offYIndexInPage %lo",_offYIndexInPage);
...
//    if (_isDetailJie) {
//        
//        NSString *js = [NSString stringWithFormat:@"function imgAutoFit() { window.location.hash = \"#%@\";}",_detailUrl];
//        js = [NSString stringWithFormat:js, [UIScreen mainScreen].bounds.size.width - 20];
//        
//        [theWebView stringByEvaluatingJavaScriptFromString:js];
//        [theWebView stringByEvaluatingJavaScriptFromString:@"imgAutoFit()"];
//        
//        _isDetailJie = NO;
////        self.calcPageOffy=1;
//    }
